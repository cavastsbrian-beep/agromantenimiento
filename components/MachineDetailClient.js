...
export default function MachineDetailClient({ id, isAdmin }) {
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-10">
      ...
      {records.length > 0 && (
        <button
          onClick={() => router.push(`/m/${id}/seguimiento/${records[0].id}`)}
          className="mb-3 flex w-full items-center justify-center gap-1.5 rounded-2xl bg-white p-4 text-sm font-semibold text-[#157347] shadow-sm hover:bg-gray-50"
        >
          <ChevronRight size={16} /> Seguimiento
        </button>
      )}
